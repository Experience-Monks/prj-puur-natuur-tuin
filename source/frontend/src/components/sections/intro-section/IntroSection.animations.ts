...
import { type IntroSectionRefs } from './IntroSection';

export function createInAnimation(refs: IntroSectionRefs): gsap.core.Animation | undefined {
...
export function createOutAnimation(refs: IntroSectionRefs): gsap.core.Animation | undefined {
  const [isValid, validatedRefs] = validateAndUnwrapRefs(refs);
  if (!isValid) {
    return undefined;
  }
