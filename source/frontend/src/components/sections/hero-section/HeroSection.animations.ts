...
import type { HeroSectionRefs } from './HeroSection.types';

export function createInAnimation(refs: HeroSectionRefs): gsap.core.Animation | undefined {
...
export function createOutAnimation(refs: HeroSectionRefs): gsap.core.Animation | undefined {
  const [isValid, validatedRefs] = validateAndUnwrapRefs(refs);
  if (!isValid) {
    return undefined;
  }
