...
import { type ProgramSectionRefs } from './ProgramSection';

export function createInAnimation(refs: ProgramSectionRefs): gsap.core.Animation | undefined {
...
export function createOutAnimation(refs: ProgramSectionRefs): gsap.core.Animation | undefined {
  const [isValid, validatedRefs] = validateAndUnwrapRefs(refs);
  if (!isValid) {
    return undefined;
  }
