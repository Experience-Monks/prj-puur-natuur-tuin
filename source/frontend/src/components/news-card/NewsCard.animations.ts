...
import { type NewsCardRefs } from './NewsCard';

export function createInAnimation(refs: NewsCardRefs): gsap.core.Animation | undefined {
...
export function createOutAnimation(refs: NewsCardRefs): gsap.core.Animation | undefined {
  const [isValid, validatedRefs] = validateAndUnwrapRefs(refs);
  if (!isValid) {
    return undefined;
  }
