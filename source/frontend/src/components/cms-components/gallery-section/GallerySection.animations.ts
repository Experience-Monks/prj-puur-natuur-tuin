...
import { type GallerySectionRefs } from './GallerySection';

export function createInAnimation(refs: GallerySectionRefs): gsap.core.Animation | undefined {
...
export function createOutAnimation(refs: GallerySectionRefs): gsap.core.Animation | undefined {
  const [isValid, validatedRefs] = validateAndUnwrapRefs(refs);
  if (!isValid) {
    return undefined;
  }
