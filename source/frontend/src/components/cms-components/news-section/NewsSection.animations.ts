...
import type { NewsSectionRefs } from './NewsSection.types';

export function createInAnimation(refs: NewsSectionRefs): gsap.core.Animation | undefined {
...
export function createOutAnimation(refs: NewsSectionRefs): gsap.core.Animation | undefined {
  const [isValid, validatedRefs] = validateAndUnwrapRefs(refs);
  if (!isValid) {
    return undefined;
  }
