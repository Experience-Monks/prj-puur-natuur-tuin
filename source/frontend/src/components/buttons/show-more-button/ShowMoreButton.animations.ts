...
import { type ShowMoreButtonRefs } from './ShowMoreButton';

export function createInAnimation(refs: ShowMoreButtonRefs): gsap.core.Animation | undefined {
...
export function createOutAnimation(refs: ShowMoreButtonRefs): gsap.core.Animation | undefined {
  const [isValid, validatedRefs] = validateAndUnwrapRefs(refs);
  if (!isValid) {
    return undefined;
  }
