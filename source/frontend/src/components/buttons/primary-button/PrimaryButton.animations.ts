...
import { type PrimaryButtonRefs } from './PrimaryButton';

export function createInAnimation(refs: PrimaryButtonRefs): gsap.core.Animation | undefined {
...
export function createOutAnimation(refs: PrimaryButtonRefs): gsap.core.Animation | undefined {
  const [isValid, validatedRefs] = validateAndUnwrapRefs(refs);
  if (!isValid) {
    return undefined;
  }
